...
OrderTime.openAddOrderTime = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: BBQ.ctxPath + '/user/order_time/order_time_add'
    });
...
OrderTime.openOrderTimeDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: BBQ.ctxPath + '/user/order_time/order_time_edit?id=' + OrderTime.seItem.id
        });
        this.layerIndex = index;
    }
};
...
OrderTime.delete = function () {
    if (this.check()) {
        $.post(BBQ.ctxPath + "/user/order_time/delete",{id:this.seItem.id}, function (data) {
            BBQ.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable(OrderTime.id, "/user/order_time/list?orderNo="+$("#orderNo").val(), defaultColunms);
