...
RestaurantInfoDlg.addSubmit = function() {
    ...
    //提交信息
    $.post(BBQ.ctxPath + "/user/restaurant/add", this.restaurantInfoData ,function(data){
        BBQ.success("添加成功!");
...
RestaurantInfoDlg.editSubmit = function() {
    ...
    //提交信息
    $.post(BBQ.ctxPath + "/user/restaurant/update",this.restaurantInfoData, function(data){
        BBQ.success("修改成功!");
        window.parent.Restaurant.table.refresh();
        RestaurantInfoDlg.close();
    });
