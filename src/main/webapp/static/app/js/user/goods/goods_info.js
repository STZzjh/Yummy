...
GoodsInfoDlg.addSubmit = function() {
    ...
    this.goodsInfoData['restaurantName'] = BBQ.selectText('restaurantId');
    this.goodsInfoData['typeName'] = BBQ.selectText('typeId');

    //提交信息
    $.post(BBQ.ctxPath + "/user/goods/add", this.goodsInfoData ,function(data){
        BBQ.success("添加成功!");
...
GoodsInfoDlg.editSubmit = function() {
    ...
    //提交信息
    $.post(BBQ.ctxPath + "/user/goods/update",this.goodsInfoData, function(data){
        BBQ.success("修改成功!");
        window.parent.Goods.table.refresh();
        GoodsInfoDlg.close();
    });
