...
Goods.initColumn = function () {
    return [
        ...
        {title: '餐厅', field: 'restaurantName', visible: true, align: 'center', valign: 'middle'},
        ...
        {title: '总量', field: 'number', visible: true, align: 'center', valign: 'middle'},
        {title: '剩余', field: 'shengyuNumber', visible: true, align: 'center', valign: 'middle'},
...
Goods.openAddGoods = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: BBQ.ctxPath + '/user/goods/goods_add'
    });
...
Goods.openGoodsDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: BBQ.ctxPath + '/user/order_goods/order_goods_add?goodsId=' + Goods.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Goods.delete = function () {
    if (this.check()) {
        $.post(BBQ.ctxPath + "/user/goods/delete",{id:this.seItem.id}, function (data) {
            BBQ.success("删除成功!");
            Goods.table.refresh();
        });
    }
...
$(function () {
    ...
    var table = new BSTable(Goods.id, "/user/goods/list", defaultColunms);
