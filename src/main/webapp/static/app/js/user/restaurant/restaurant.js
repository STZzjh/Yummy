...
Restaurant.openAddRestaurant = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: BBQ.ctxPath + '/user/restaurant/restaurant_add'
    });
...
Restaurant.openRestaurantDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: BBQ.ctxPath + '/user/restaurant/restaurant_edit?id=' + Restaurant.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Restaurant.delete = function () {
    if (this.check()) {
        $.post(BBQ.ctxPath + "/user/restaurant/delete",{id:this.seItem.id}, function (data) {
            BBQ.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable(Restaurant.id, "/user/restaurant/list", defaultColunms);
