...
RestaurantInfoDlg.addSubmit = function() {
    ...
    //提交信息
    $.post(BBQ.ctxPath + "/restaurant/restaurant/add", this.restaurantInfoData ,function(data){
        BBQ.success("添加成功!");
...
RestaurantInfoDlg.editSubmit = function() {
    ...
    var sendData = this.restaurantInfoData;
    // this.restaurantInfoData['typeName'] = BBQ.selectText('typeId');
    BBQ.confirm("确认修改，Yummy平台会重新审核你的餐厅，届时将不能登录",function () {
        //提交信息
        $.post(BBQ.ctxPath + "/restaurant/profile/update",sendData, function(data){
            BBQ.success("修改成功!");
            window.parent.Restaurant.table.refresh();
            RestaurantInfoDlg.close();
        });
